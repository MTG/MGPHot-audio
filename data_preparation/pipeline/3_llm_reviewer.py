...
model = "qwen2.5:32b"
INPUT_PATH = "no_matches.json"
OUTPUT_DIR = Path(f"outputs_ollama/qwen2.5_32b_final")

# Prompts
REASONING_PROMPT = """
You are an expert in identifying official versions of songs on YouTube based on video titles.
...
Explain whether the artist and song title in the video title match the given artist and title.
Provide your reasoning clearly and step by step, but do not make a final decision yet.
"""

MATCH_DECISION_PROMPT = """
Based on your reasoning above, do the artist and song title in the video title match the given track?

Answer only YES or NO.
"""

EXCLUSION_PROMPT = """
Does the video title "{video_title}" contain any of the following or similar words:
...
for track in tqdm(tracks_sub):
    ...
    for video in track.get("youtube", []):
        ...
        if decision_result:
            ...
            exclusion_prompt = EXCLUSION_PROMPT.format(video_title=video_title)
