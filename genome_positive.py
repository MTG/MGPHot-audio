...
def values_to_tags(values, thr=0.8):
    tags = []
    for i, v in enumerate(values):
        # Special rule for tonality
...
def convert_file(in_path: Path, out_path: Path, thr=0.8):
    ...
    for k, item in data.items():
        ...
        if isinstance(values, list):
            ...
            item["positive_tags"] = values_to_tags(values, thr=thr)
...
def main():
    parser = argparse.ArgumentParser(description="Convert gene_values to positive_tags.")
    parser.add_argument("-i", "--input", type=Path, default=Path("genome_index_split.json"),
                        help="Input JSON path.")
    parser.add_argument("-o", "--output", type=Path, default=Path("genome_index_split_positive.json"),
                        help="Output JSON path.")
    parser.add_argument("--thr", type=float, default=0.8,
                        help="Threshold for general tags (value > thr).")
    parser.add_argument("--minor_thr", type=float, default=0.2,
                        help="Threshold for tag Minor (value < minor_thr).")
    args = parser.parse_args()

    convert_file(args.input, args.output, thr=args.thr)
